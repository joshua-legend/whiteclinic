...
const CScheduleTimeMapping = ({
  selectDate,
  orderInfo,
  engineerName,
}: CustomerTimeMappingProps) => {
  return (
      ...
      {workTimeLine && workTimeLine.length > 0 ? (
        workTimeLine.map((timeSlot, index) => {
          const customer = orderInfo?.find(
            (order) =>
              order.appointmentTime === timeSlot.label &&
              order.appointmentDate === selectDate &&
              order.assignedEngineer === engineerName
          );
